...
ths = [threading.Thread(target=build, args=(i,)) for i in range(3)]
...
res = {}
ths = [threading.Thread(target=turn, args=(i, res)) for i in range(3)]
[t.start() for t in ths]; [t.join() for t in ths]
print("RESULT c3-64K:", [f"{res[i]:.1f}" for i in range(3)], flush=True)
import subprocess
